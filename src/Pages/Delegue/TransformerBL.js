...
function TransformerBL() {
  ...
  /** end crop img**/
  function setFour(four,newDateFormat,idDelegue,numero,address){
    var array = [];
    array.push({
      Adresse: address,
      Client: null,
    ...
    async (four, entities) => {
      dispatch(cheeckFournisseur({ nomFournisseur: four.Fournisseur })).then(
        (rowsdes) => {
          ...
          var numero = entities[0].NumeroBL;
          ...
          setAdresse(address);
          setFour(four,newDateFormat,idDelegue,numero,address)
          /* var entete = [];
            entete.push({
            Adresse: null,
            Client: null,
  ...
  const removeFournisseur = useCallback(async (list,id) => {
    setFour(null,list[0].dateBL,list[0].iduser,list[0].numeroBL,list[0].Adresse);
    setFournisseur(null);
  },
    []
  );
