...
function AjouterPharmacie() {
  ...
  function submitForm(event) {
    ...
    if (
      ...
      dispatch(getPharmacieByNum({ id:id,code:code })).then(e=>{
        if(e.payload !== true){
          dispatch(pharmacieAdded({ nom,adresse,idIms,telephone,email, id,code,lng,lat })).then(e=>{
            if(e.payload ===true){
          ...
          /* dispatch(pharmacieAdded({ nom,adresse,idIms,telephone,email, id,code,lng,lat }));
          if (isNaN(location.id) === true) {
            notify("tr", "Insertion avec succes", "success");
          } else {
            notify("tr", "Modifier avec succes", "success");
  ...
  }, [location.id,dispatch,getIms,navigate]);
